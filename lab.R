...
#' A3.1. It's a data frame with 588 rows and 6 columns.
...
#' A3.2. They are in inflation-adjusted (constant) dollars, so no we don't need to consider inflation.
...
rd_plot + facet_wrap(~department,scales='free_y')
...
rd_plot + facet_wrap(~department,scales='free_y')+labs(x='Year',y='Federal R&D Spending',title='Federal R&D Spending Over Years by Funding Agency/Department')
...
#' A5.2. Create a new variable as R&D spending adjusted for GDP.
 
#' 3. How does this line violate the rules of functional programming? How could it be modified to avoid the violation? 
#' A5.3. It violates immutability: once the value is assigned to a variable, that value cannot be changed, but I guess by creating a new variable we can basically avoiding the violation.
...
rd_plot <-  ggplot(data = dataf, aes(x = year, y = rd_per_gdp,
    ...
    geom_line()
rd_plot+facet_wrap(~department,scales='free_y')+
    labs(x='Year',y='Federal R&D Spending per GDP',title='Federal R&D Spending per GDP Over Years by Funding Agency/Department')
